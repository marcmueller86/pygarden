...
class VisualizeData(object):
    ...
    def create_export(self, file_name):
        ...
        for i, sensor in enumerate(df['name'].unique()):
        ...
        fig.add_trace(
            ...
            header=dict(values=['Sensor','Datum','Leitfähigkeit(us/cm)', 'Lichtstärke (Lux)', 'Feuchtigkeit (?%)', 'Temperatur (C°)', 'Batterie Ladung (%)'],
                        align='left'),
            cells=dict(values=[df.name, df.datetime, df.conductivity.apply(lambda x: round(x, decimals)), df.light.apply(lambda x: round(x, decimals)), df.moisture.apply(lambda x: round(x, decimals)), df.temperature.apply(lambda x: round(x, decimals)), df.battery],
                    align='left')),
                    row=3, col=1
        )
    ...
    def create_scatter_trace(self, color, df, fig, y, name, row, col, legend):
        fig.add_trace(
            go.Scatter(x=df['datetime'], y=df[y],name=name, line=dict(width=2, color=color),showlegend=legend),
            row=row, col=col
        )
        return fig
